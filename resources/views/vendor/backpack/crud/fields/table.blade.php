...
<div ng-app="backPackTableApp" ng-controller="tableController" @include('crud::inc.field_wrapper_attributes') >
    ...
    <div class="array-container form-group">
        <table class="table table-bordered table-striped m-b-0" ng-init="field = '#{{ $field['name'] }}'; items = {{ $items }}; table_name = '{{$field['name']}}'; max = {{$max}}; min = {{$min}}; maxErrorTitle = '{{trans('backpack::crud.table_cant_add', ['entity' => $item_name])}}'; maxErrorMessage = '{{trans('backpack::crud.table_max_reached', ['max' => $max])}}'">
            ...
            <tbody ui-sortable="sortableOptions" ng-model="items" class="table-striped">
                ...
                <tr ng-repeat="item in items" class="array-row">
                    ...
                    <td>
                        @if(isset($field['columns_property']["$prop"]['type']) && \View::exists('vendor.backpack.crud.fields.table.'.$field['columns_property']["$prop"]['type']) )
                            @include('vendor.backpack.crud.fields.table.'.$field['columns_property']["$prop"]['type'], [
                                'table_field_property' => $field['columns_property']["$prop"]
                            ])
                        @else
            ...
            window.angularApp.controller('tableController', function($scope){
                ...
                $scope.removeItem = function(item){
                    ...
                    window.setTimeout( function(){
                        var fname = $scope.table_name + '_removeItem_after';
